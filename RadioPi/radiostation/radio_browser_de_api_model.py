...
class RadioBrowserDeClient:
    ...
    def find_stations_by(self, category, name):
        return self.request_and_map(
            "http://95.179.139.106/json/stations/by%s/%s" % (category, name), 
            self.mapper.as_station)

    def find_stations_by_topvote(self, rows):
        if (not rows):            
            return self.request_and_map(
                "http://95.179.139.106/json/stations/topvote", 
                self.mapper.as_station)
        else:
            return self.request_and_map(
                "http://95.179.139.106/json/stations/topvote/%s" % rows, 
                self.mapper.as_station)
    ...
    def get_list_of_item(self, category, filter_string): 
        if (filter_string):
            return self.request_and_map(
                "http://95.179.139.106/json/%s/%s" % (category, filter_string),
                self.mapper.as_list_of_item)
        else:
            return self.request_and_map(
                "http://95.179.139.106/json/%s" % category,
                self.mapper.as_list_of_item)

    def get_playable_url(self, station_id):
        try:
            response = requests.get("http://95.179.139.106/json/byurl/%s" % station_id)
